...
`/api/parts` is only a local dev prefix so those mock calls stop colliding with Studio Concierge on `/api` → `:8000`; Vite rewrites `/api/parts` → `/api` and proxies to `VITE_SIMPLEPARTS_BACKEND_URL` (default `http://127.0.0.1:5001`, or the origin in `app/.env.remote` when you run `npm run dev:remote`). It is not a platform route and nothing should depend on it.
