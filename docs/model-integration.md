...
| [app/src/lib/appChat.ts](../app/src/lib/appChat.ts) + `registerAppIntake` in workspace | Concierge opens/zooms an app window then delivers text/files via a WorkspaceProvider ref into the mounted page; app replies echo into the Studio thread | `SuggestedAction` accept → BFF starts the app turn; UI only renders streamed messages |
